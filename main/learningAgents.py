...
class ReinforcementAgent(ValueEstimationAgent):
  ...
  def registerInitialState(self, state):
    ...
    if self.episodesSoFar == 0:
        print('Beginning ', self.numTraining, 'episodes of Training')
  ...
  def final(self, state):
    ...
    if self.episodesSoFar % NUM_EPS_UPDATE == 0:
        ...
        if self.episodesSoFar <= self.numTraining:
            ...
            print ('Completed: ',self.episodesSoFar, ' out of: ', self.numTraining, 'training episodes')
            print ('Average Rewards over all training:', trainAvg)
        else:
            testAvg = float(self.accumTestRewards) / (self.episodesSoFar - self.numTraining)
            print ('Completed', (self.episodesSoFar - self.numTraining),  'test episodes' )
            print ('Average Rewards over testing: ', testAvg)
        print ('Average Rewards for last', NUM_EPS_UPDATE, 'episodes:' ,windowAvg)
        print ('Episode took ', (time.time() - self.episodeStartTime), 'seconds')
        self.lastWindowAccumRewards = 0.0
        self.episodeStartTime = time.time()
