...
class avdQLearningAgent(ReinforcementAgent):
  """
    Q-Learning Agent
...
class CrossQLearningAgent(avdQLearningAgent):
  ...
  def __init__(self, epsilon=0.05,gamma=0.8,alpha=0.2, numTraining=0, **args):
    ...
    avdQLearningAgent.__init__(self, **args)
...
class QLearningAgent(ReinforcementAgent):
  ...
  def update(self, state, action, nextState, reward):
    ...
    print("QVALUE", self.getQValue(state, action))
    print("VALUE", self.getValue(nextState))
    self.qValues[(state.__hash__(), action)] = self.getQValue(state, action) + self.alpha * (reward + self.discount * self.getValue(nextState) - self.getQValue(state, action))
